function [Vfc_conv,Ifc_boost,f] = exported_lpc_deep(Vfc ,Ifc ,Vdc,Ifc_out,ILmax,Vdc_ref,Ipv_out,SOC,time_data,...
...
%         ub = [1100];
        lb = [max(0,x0-dPfc)];
        ub = [min(1100,x0+dPfc)];
        Pwant = Pwanted - Ipv_out*Vdc;
        Pfc = opti_fc_lmc_deep(Ts,Tot_sample,Num_sample,x0,lb,ub,SOC,Pwant,@fc_powertrain_fun,Vdc,Ifc_out,ILmax,Vdc_ref,FCdata,...
